...
for i in pbar((i for i in range(nt))):
    ...
    plt.text(0.49, 62, f"t: {i*dt:0.2f}s", size=15,
             ha="center", va="center",
...
plt.xlabel('Distancia [m]')
plt.ylabel('Temperatura [°C]')
lines = [0, 30, 80, 250, 500, 1000, 1799]
for i in range(len(lines)):
    plt.plot(x, Tdist[lines[i], :], c=copper, linewidth='1')
    ax.annotate(f't={lines[i]*dt:0.0f}s',
                xy=(x[5+(i*6)], Tdist[lines[i], 5+(i*6)]-0.6), xycoords='data',
                xytext=(0.05+(0.13*i), 55-(0.01*i)), textcoords='data',
