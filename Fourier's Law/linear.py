...
for i in pbar(i for i in range(nt)):
    ...
    plt.plot(x, Tdist[i, :], c='#A06448', linewidth='3.0')
...
plt.xlabel('Distancia [m]')
plt.ylabel('Temperatura [°C]')
lines = [0, 5, 20, 40, 70, 110, 264]
for i in range(len(lines)):
    plt.plot(x, Tdist[lines[i], :], c=copper, linewidth='0.8')
    ax.annotate(f't={lines[i]*dt:0.2f}s',
                xy=(x[2+(i*3)], Tdist[lines[i], 2+(i*3)]+0.6), xycoords='data',
                xytext=(0.05+(0.04*i), 32-(6*i)), textcoords='data',
